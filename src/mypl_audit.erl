...
% TODO: remove once auditdata is fixed
fix_articleaudit() ->
    % fixes some broken records generated by wrong correction code
    mypl_db_util:do_trans(qlc:q([fix_articleaudit_helper(X#articleaudit.text, X) || X <- mnesia:table(articleaudit)])),
    ok.

% TODO: remove once auditdata is fixed
fix_articleaudit_helper("Korrektur auf " ++ L, R) ->
    erlang:display({L, R}),
    mnesia:dirty_delete(articleaudit, R#articleaudit.id);
fix_articleaudit_helper(_, _) ->
    ok.
...
fix_unitaudit() ->
    % fixes some broken records generated by wrong correction code
    mypl_db_util:do_trans(qlc:q([fix_unitaudit_helper(X#unitaudit.text, X) || X <- mnesia:table(unitaudit)])),
    ok.

% TODO: remove once auditdata is fixed
fix_unitaudit_helper("Korrektur von " ++ L, R) ->
    erlang:display({L, R});
    % mnesia:dirty_delete(unitaudit, R#unitauditaudit.id);
fix_unitaudit_helper(_, _) ->
    ok.
